...
legend_elements = [
    ...
    Line2D([0], [0], color='red', lw=2.5, label=r'Singular heteroclinic orbit $\Gamma$')
]
...
# Plot the singular heteroclinic orbit
# Stage 1: along M_0V from (1,0,0) to (0,0,W*)
U_het = np.linspace(1, 0.02, 100)
V_het = np.zeros_like(U_het)
W_het = -U_het  # approximate W=-U along the orbit
ax.plot(U_het, V_het, W_het, color='red', lw=2.5, 
        zorder=5, label=r'Singular orbit $\Gamma$')

# Stage 2: fast jump along M_0U from (0,0,W*) to (0,V_inf,0)
V_jump = np.linspace(0, 0.5, 100)
U_jump = np.zeros_like(V_jump)
W_jump = np.linspace(-0.02, 0, 100)  # W decreasing to 0
ax.plot(U_jump, V_jump, W_jump, color='red', lw=2.5,
        ls='--', zorder=5)

# Add arrow to show direction
ax.quiver(0.5, 0, -0.5, -0.3, 0, 0.3, 
